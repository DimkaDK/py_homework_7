...
def print_operation_table(operation, num_rows = 7, num_columns = 7):
    numbers_in_row = [i for i in range(1, num_columns + 1)]
    numbers_in_column = [i for i in range(1, num_rows + 1)]
    ...
    for x in range(0, num_rows):
        if x == 0:
            ...
            for y in range(0, num_columns):
                if y == 0:
                   numbers_in_row[y] = numbers_in_column[x] 
                else:
                   numbers_in_row[y] = operation(x + 1, y + 1)
            print(*numbers_in_row)
